    ...
    <div class="install-card">
      ...
      <div class="install-content">
        ...
        <div v-if="currentStep === 1" class="step-content">
          <n-form ref="dbFormRef" :model="dbForm" :rules="dbRules" label-placement="top">
            <n-form-item label="数据库类型" path="engine">
              <n-select
                v-model:value="dbForm.engine"
            ...
            <!-- MySQL/PostgreSQL 配置 -->
            <template v-if="['mysql', 'postgresql'].includes(dbForm.engine)">
              <n-grid :cols="2" :x-gap="16">
                ...
                <n-grid-item>
                  <n-form-item label="端口" path="port">
                    <n-input-number v-model:value="dbForm.port" :min="1" :max="65535" class="w-full" />
                  </n-form-item>
                </n-grid-item>
              </n-grid>
              
              <n-form-item label="数据库名" path="name">
                <n-input v-model:value="dbForm.name" placeholder="cronJob" />
              </n-form-item>
              
              <n-grid :cols="2" :x-gap="16">
                <n-grid-item>
                  <n-form-item label="用户名" path="user">
                    <n-input v-model:value="dbForm.user" placeholder="root" />
                  </n-form-item>
                </n-grid-item>
                <n-grid-item>
                  <n-form-item label="密码" path="password">
                    <n-input v-model:value="dbForm.password" type="password" show-password-on="click" placeholder="数据库密码" />
                  </n-form-item>
                </n-grid-item>
              </n-grid>
            </template>
            ...
            <!-- SQLite 配置 -->
            <template v-if="dbForm.engine === 'sqlite'">
              <n-form-item label="数据库文件" path="name">
                <n-input v-model:value="dbForm.name" placeholder="cronJob" />
        ...
        <div v-if="currentStep === 3" class="step-content">
          ...
          <n-descriptions bordered :column="1">
            <n-descriptions-item label="数据库类型">
              {{ getDbEngineLabel(dbForm.engine) }}
            </n-descriptions-item>
            <n-descriptions-item v-if="['mysql', 'postgresql'].includes(dbForm.engine)" label="连接地址">
              {{ dbForm.host }}:{{ dbForm.port }}
            </n-descriptions-item>
            <n-descriptions-item label="数据库名">
              {{ dbForm.name }}
            </n-descriptions-item>
            <n-descriptions-item v-if="['mysql', 'postgresql'].includes(dbForm.engine)" label="用户名">
              {{ dbForm.user }}
            </n-descriptions-item>
...
// 表单验证规则
const dbRules = {
  engine: { required: true, message: '请选择数据库类型' },
  host: { required: true, message: '请输入主机地址' },
  port: { required: true, type: 'number' as const, message: '请输入端口号' },
  name: { required: true, message: '请输入数据库名' },
...
const testDbConnection = async () => {
  ...
  try {
    ...
    if (['mysql', 'postgresql'].includes(dbForm.engine)) {
      params.host = dbForm.host;
      params.port = dbForm.port;
      params.user = dbForm.user;
      params.password = dbForm.password;
    }
...
const startInstall = async () => {
  ...
  try {
    ...
    if (['mysql', 'postgresql'].includes(dbForm.engine)) {
      params.database.host = dbForm.host;
      params.database.port = dbForm.port;
      params.database.user = dbForm.user;
      params.database.password = dbForm.password;
    }
